...
class StressPreferenceScreen extends HookWidget {
  const StressPreferenceScreen({super.key});
  ...
  Widget build(BuildContext context) {
    ...
    final moodPresets = useMemoized(() {
      ...
      sorted.sort((a, b) {
        ...
        if (aLabel == 'surprise') return 1; // push 'surprise me' down
        if (bLabel == 'surprise') return -1;

        return aLabel.compareTo(bLabel); // normal alphabetical sort
      });
    ...
    void goNext() async {
      ...
      try {
        await profileApi.clearStressfulPresets();
        await profileApi.selectStressfulPreset(
            presetId: ProfileSelectStressfulPresetMutation(
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 20),
              const Text(
                "If you are sad or stressed, \nHow should the app help you find the perfect songs?",
                style: TextStyle(
              ...
              Expanded(
                child: Scrollbar(
                  ...
                  thumbVisibility: true, // always show thumb (optional)
                  thickness: 4, // optional styling
                  radius: const Radius.circular(10),
                  child: SingleChildScrollView(
                    controller: scrollController,
                    child: Container(
                      width:
                          double.infinity, // ✅ Force full width inside scroll
                      child: Wrap(
                        spacing: 10,
                        runSpacing: 10,
              ...
              const SizedBox(height: 20),
              NextButton(
                enabled: selectedGenres.value.isNotEmpty,
                onPressed: goNext,
              ),
              const SizedBox(height: 20),
            ],
